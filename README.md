...
- **Forge unfreeze (1.1.4)**: clicking FORGE only switches the panel. The pipeline no longer runs on the click frame; layout is shrink-safe; evaluation yields so the renderer stays interactive.
...
## Release 1.1.4 — Windows, macOS, Steam Deck
...
| --- | --- | --- |
| **Windows** | `AGI-PRIME-1.1.4-Windows-Setup-x64.exe` | Installer with Start Menu + desktop shortcut. Portable: `AGI-PRIME-1.1.4-Windows-Portable-x64.exe`. |
| **macOS** | `AGI-PRIME-1.1.4-macOS-arm64.dmg` (Apple Silicon) or `…-x64.dmg` (Intel) | First open: right-click the app → **Open** (unsigned build). |
| **Steam Deck / Linux** | `AGI-PRIME-1.1.4-linux-x86_64.AppImage` | Desktop Mode: mark executable and double-click. Game Mode: add as a non-Steam game, or use `scripts/steamdeck-launch.sh`. Also ships a `.deb`. |
...
CI: `.github/workflows/release.yml` builds all three OS families on version tags (`v1.1.4`) or **workflow_dispatch**.
