...
export function runHardeningCheck(input: HardeningInput): HardeningReport {
  ...
  items.push(check(
    ...
    input.conscienceEnabled ? 'Conscience is active and monitoring actions.' : 'Conscience is DISABLED — actions will not be ethically screened.',
  ));
  ...
  const overallStatus: CheckStatus =
    items.some(i => i.status === 'fail') ? 'fail'
